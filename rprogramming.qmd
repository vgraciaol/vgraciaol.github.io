...
A **matrix** is a powerful tool, featuring a rectangular arrangement of numbers, symbols, or expressions in rows and columns.
...
Matrices are useful for organizing (numeric) data based on individuals (rows) and attributes (columns). For example, imagine you have the age, sex, and years of schooling for three people:

```{r}
#| echo: false

df <- data.frame(Age  = c(48,34,56),
                 Male = c(1,0,1),
                 Sch_yr = c(10,9,12))
           

knitr::kable(df, format = "html", 
             col.names = c("Age","Male","Years of schooling"))
```
...
# if a and b are number then return the sum
# otherwise, return NA
...
run_Markov <- function(l_params){
    ...
    # Create a matrix containing the transition probability matrix. 
    # Each matrix represents the transition between states 
    # and the third dimension represents the time step at 
    # which that matrix applies
    a_m_T <- array(NA, dim = c(n_states,n_states,n_cycles))
    for(time_i in v_times){ # time_i = 1
      a_m_T[ , , time_i] <- m_T
    }
    
    # Create the transition trace matrix (m_TR) capturing 
    # the proportion of the cohort in each state at each time point
